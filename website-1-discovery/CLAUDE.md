...
2. **D2: Project Research** -- Deep-dive intelligence across 8 research domains, informed by D1 (including interview answers) and the project's language configuration. The operator confirms which languages to research in before dispatch; the researcher agent adapts query depth per domain type. One generic researcher agent dispatched per domain (max 2 concurrent), each loading a domain-specific methodology file. Domains: R1 SERP & Search Landscape, R2 Competitor Landscape, R3 Audience & User Personas, R4 UX/UI Patterns & Benchmarks, R5 Content Landscape & Strategy, R6 Reputation & Social Proof, R7 Technology & Performance, R8 Industry & Market Context. Wave 1 domains run independently; Wave 2 domains (R4, R5) benefit from Wave 1 outputs. Each produces an R-document; all consolidate into D2 summary with cross-topic findings, strategic opportunities, and proposal inputs.
3. **D3: Project Brief** -- Consolidated proposal: current state, strategic guidance, proposed solution. Specific enough to quote from.

**D1 lifecycle:** D1 has two states: `research-complete` (set automatically after client-intake finishes) and `interview-complete` (set manually after the operator fills in client answers). D2 research is more targeted after `interview-complete`, and D3 warns if only `research-complete`.

## Available Capabilities

- **Skills:** project-init (initialize project + state tracking), client-intake (research + D1 generation), project-research (D2 research orchestration -- 8 domains in waves of 3), project-brief (synthesize D1 + D2 into D3 proposal), dispatch-subagent (agent orchestration via Task tool)
...
- **Language configuration:** Primary language and market set at project init, stored in project-state.md. Operator confirms research languages before D2 dispatch. Researcher agents adapt queries -- primary language gets deepest research, additional languages get lighter coverage. Per-domain strategy varies by topic type (local-market domains get full multilingual treatment, technical domains are language-independent).
- **Project state:** `project-state.md` in the project directory tracks pipeline progress and document status
- **References:** D1 template, R-document template, D3 template, questioning strategy guide, domain quick-reference, 8 research domain methodology files, crawl method specs
