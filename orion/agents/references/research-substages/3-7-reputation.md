# Substage 3.7 — Reputation & Social Proof

**Code:** R7
**Slug:** Reputation
**Output:** `research/R7-Reputation.txt`
**Hypothesis:** Client lacks trust signals that competitors have established
**Dependencies:** R4-Competitors, R5-Market, R6-Audience
**Reads from:** `project.json`, `baseline-log.txt`, `research/R4-Competitors.txt`, `research/R5-Market.txt`, `research/R6-Audience.txt`
...
Review samples serve two purposes: direct input for messaging in Concept Creation and Proposal (what customers praise and complain about is positioning intelligence), and validation of persona objections defined in R6-Audience.
...
R4 provides the competitor roster with tier classifications. R5 provides trust signal requirements for this industry (R5 identifies what trust signals matter — this stage checks who has them). R6 provides persona trust thresholds and objections (R6 defines what each persona needs to see — this stage checks whether it exists). Read all three before starting.
...
- Website trust signals — case studies (count, structure quality, specificity), testimonials (count, placement, format, credibility), other credibility markers (awards, certifications, memberships, partner logos, client logos, media mentions, team credentials, trust badges). Cross-referenced with R5's industry trust requirements and R6's persona trust thresholds
- Persona validation — compare review praise/complaint patterns against R6's persona objections. Flag mismatches where real customer voice contradicts persona assumptions
...
From `research/R4-Competitors.txt`: competitor roster with tier classifications, zone map.
From `research/R5-Market.txt`: trust signal requirements for this industry (what the market demands).
From `research/R6-Audience.txt`: persona trust thresholds and objections (what each persona needs to see to convert).
...
Persona validation checkpoint: compare praise/complaint patterns against R6's persona objections. If the primary persona's objection is "worried about price" but reviews never mention price and consistently complain about communication speed, flag the mismatch — persona may need updating before Concept Creation relies on it.
...
Cross-reference with R5: check the trust signal requirements R5 identified. If R5 says "case studies are table-stakes," flag any site lacking them. If R5 says "regulatory registration numbers required," check whether each site displays them. Cross-reference with R6: check each persona's trust threshold. If primary persona needs "case studies + team photos + certifications," which sites satisfy that and which don't?
...
Classify each gap: Critical (client lacks what all direct threats have AND primary persona requires — must address), Competitive (below competitor average but not missing a universal standard — should address), Opportunity (no competitor has what the audience expects based on R5 and R6 — differentiation for the proposal), Strength (client has what competitors lack — preserve and amplify, feature prominently in proposal).
...
Write `research/R7-Reputation.txt`. Apply the decision framework and formatting rules. Append key findings to `baseline-log.txt` tagged with `[R7]`.
...
- Persona validation notes → R6-Audience update if mismatches found (before Concept Creation relies on personas)
...
- Social tone and format signals → R10-Content (content strategy), Concept Creation (voice direction)
- Trust element placement needs → R9-UX (where credibility markers go on pages)
